...
export default function MessagingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-14 flex">
      ...
      <div className="w-72 bg-white border-r shrink-0 flex flex-col">
        ...
        <div className="overflow-y-auto flex-1">
          {connectedUsers.length === 0 ? (
            <p className="text-gray-400 text-sm p-4">Koi connection nahi hai abhi</p>
          ) : (
      ...
      <div className="flex-1 flex flex-col">
        {!selectedUser ? (
          <div className="flex-1 flex items-center justify-center text-gray-400">
            Kisi ko select karo baat karne ke liye
          </div>
        ) : (
          <>
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3">
              {messages.length === 0 && (
                <p className="text-center text-gray-400 text-sm mt-10">
                  Abhi tak koi message nahi — pehle message karo!
                </p>
              )}
              {messages.map((m) => {
                ...
                return (
                  <div key={m._id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${isMe ? "bg-blue-600 text-white rounded-br-none" : "bg-white text-gray-800 shadow rounded-bl-none"}`}>
                      {m.text}
                    </div>
                  </div>
                )
